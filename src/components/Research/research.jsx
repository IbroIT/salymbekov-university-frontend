...
const Research = () => {
  ...
  const fetchPublications = async () => {
    try {
      const response = await fetch('https://su-back.onrender.com/research/api/publications/');
      if (!response.ok) {
  ...
  const fetchConferences = async () => {
    try {
      const response = await fetch('https://su-back.onrender.com/research/api/conferences/upcoming/');
      if (!response.ok) {
        throw new Error(`HTTP error! status: ${response.status}`);
      }
      const data = await response.json();
