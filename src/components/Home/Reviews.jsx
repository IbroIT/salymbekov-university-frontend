      ...
      <div className="flex justify-center md:justify-start">
        {[...Array(5)].map((_, i) => {
          ...
          return (
            <svg
              key={i}
              className={`w-4 h-4 md:w-5 md:h-5 ${ratingValue <= rating ? 'text-yellow-400' : 'text-gray-300'}`}
              fill="currentColor"
              viewBox="0 0 20 20"
            >
        ...
        <span className="ml-2 text-blue-800 font-semibold text-sm md:text-base">{rating}</span>
      </div>
    );
  };
