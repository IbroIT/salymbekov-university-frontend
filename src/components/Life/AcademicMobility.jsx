              ...
              {data.exchange_opportunities && data.exchange_opportunities.length > 0 ? (
                data.exchange_opportunities.map((opportunity, index) => (
                  <div key={opportunity.id || index} className="bg-white rounded-lg shadow-md p-6 border">
                    <h3 className="text-xl font-semibold text-gray-900 mb-3">{opportunity.title || 'Возможность обмена'}</h3>
                    <p className="text-gray-600 mb-4">{opportunity.description || 'Описание не указано'}</p>
                    
                    <div className="mb-4">
                      <div className="flex items-center text-sm text-gray-500 mb-3">
                        <CalendarDaysIcon className="w-4 h-4 mr-2" />
                        <span>Продолжительность: {opportunity.duration || 'Не указано'}</span>
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <ul className="text-sm text-gray-600 space-y-1">
                        {opportunity.benefits && opportunity.benefits.length > 0 ? (
                          ...
                          <li className="text-gray-500">Преимущества не указаны</li>
                        )}
                      </ul>
                    </div>
                  </div>
                ...
                <div className="col-span-full text-center py-8">
                  <p className="text-gray-500">Данные о возможностях обмена пока не загружены</p>
                </div>
              )}
            </div>
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              {data.partner_universities && data.partner_universities.length > 0 ? (
                  ...
                  <div key={university.id} className="bg-white rounded-lg shadow-md p-6 border">
                    <div className="flex items-start space-x-4 mb-4">
                      <div className="w-16 h-16 bg-gray-100 rounded-lg flex items-center justify-center flex-shrink-0">
                        {university.logo ? (
                          <img src={university.logo} alt={university.name || 'University'} className="w-14 h-14 object-contain" />
                        ) : (
                          <AcademicCapIcon className="w-8 h-8 text-gray-400" />
                        )}
                      </div>
                      <div className="flex-1">
                        <h3 className="text-lg font-semibold text-gray-900 mb-1">{university.name || 'Название университета'}</h3>
                        <div className="flex items-center text-sm text-gray-600">
                          <GlobeAltIcon className="w-4 h-4 mr-1" />
                          <span>{university.city || 'Город'}, {university.country || 'Страна'}</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="pt-3 border-t">
                        ...
                        <p className="text-sm text-blue-600">{university.contact_email || 'Не указано'}</p>
                      </div>
                    </div>
                  </div>
                ))
                ...
                <div className="col-span-full text-center py-8">
                  <p className="text-gray-500">Данные о университетах-партнерах пока не загружены</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="space-y-6">
            {data.participation_requirements && data.participation_requirements.length > 0 ? (
                  ...
                  <div key={index} className="bg-white rounded-lg shadow-md p-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {section.requirements && section.requirements.length > 0 ? (
                        ...
                        <p className="text-gray-500 col-span-full">Требования не указаны</p>
                      )}
                    </div>
                  </div>
                );
              ...
              <div className="text-center py-8">
                <p className="text-gray-500">Данные о требованиях пока не загружены</p>
              </div>
            )}
